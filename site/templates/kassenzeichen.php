  ...
  <div class="container">
    ...
    if (kirby()->request()->is('POST') && get('submit')) {
      try {
        ...
        $betrag = get('betrag'); // Check if values are not null before using esc()
        $data = [
          'kassenzeichen' => is_string($kassenzeichen)
            ? esc($kassenzeichen)
            : '',
        ...
        echo "<a href='" .
          $qrCodeDataUri .
          "' download='$filename'>QR-Code herunterladen</a>";
      } catch (Exception $e) {
        $error = $e->getMessage();
      }
    }
    ...
    <form method="post">
      ...
      <div class="space-y-1">
        ...
        <div class="relative">
          ...
          <input type="text" id="betrag" name="betrag" value="<?php echo isset(
            $data['betrag'],
          )
            ? $data['betrag']
            : ''; ?>" required pattern="[0-9]+([,\.][0-9]+)?" title="Bitte gebe einen richtigen Betrag an" placeholder="333" class="block w-full rounded-lg border border-gray-200 py-3 pl-20 pr-5 leading-6 placeholder-gray-500 focus:border-indigo-500 focus:ring focus:ring-indigo-500 focus:ring-opacity-50 dark:border-gray-600 dark:bg-gray-800 dark:placeholder-gray-400 dark:focus:border-indigo-500" />
        </div>
        <div>
